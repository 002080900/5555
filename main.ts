namespace SpriteKind {
    ...
    export const sesire = SpriteKind.create()
}
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if (backgroung == 0) {
        mySprite = sprites.create(assets.image`myImage2`, SpriteKind.Player)
        sprites.create(assets.image`myImage2`, SpriteKind.Player).sayText("歡迎來到北投來泡溫泉", 2000, false)
        pause(2000)
        sprites.create(assets.image`myImage2`, SpriteKind.Player).sayText("我是這邊的猛男消防隊教你防災觀念和北投歷史", 2000, false)
        pause(2000)
        sprites.create(assets.image`myImage2`, SpriteKind.Player).sayText("來猜拳比賽贏我的會告訴你", 2000, false)
        pause(2000)
        sprites.create(assets.image`myImage2`, SpriteKind.Player).startEffect(effects.hearts, 2000)
        pause(2000)
        sprites.create(assets.image`myImage2`, SpriteKind.Player).sayText("按b鍵準備開始囉", 2000, false)
        pause(2000)
        game.reset()
    } else {
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.paper, function (sprite, otherSprite) {
    if (backgroung == 1) {
        if (controller.A.isPressed()) {
            sprites.create(assets.image`myImage5`, SpriteKind.Player).sayText("你確定", 5000, false)
            if (controller.up.isPressed()) {
                stone2 = true
                game2 = Math.percentChance(33.3)
                if (game2 == true) {
                    game.splash("恭喜獲勝")
                    選擇題()
                } else if (game2 == false) {
...
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    if (backgroung == 0) {
        遊戲()
        backgroung += 1
    } else if (backgroung == 1) {
    	
    }
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.sesire, function (sprite, otherSprite) {
    if (backgroung == 1) {
        if (controller.A.isPressed()) {
            sprites.create(assets.image`myImage5`, SpriteKind.Player).sayText("你確定", 5000, false)
            if (controller.up.isPressed()) {
...
function 選擇題 () {
    ...
    tiles.destroySpritesOfKind(SpriteKind.stone)
    tiles.destroySpritesOfKind(SpriteKind.sesire)
    ...
    mySprite2.sayText("恭喜你通關了猜拳遊戲接著是我們的選擇題", 500, false)
    game.showLongText("電腦老師陳晉是這次的肌肉猛男消防員如果遇到地震第一時間要不要開窗", DialogLayout.Bottom)
    if (game.ask("(a)對", "(B)錯")) {
        game.over(true)
    } else {
        game.over(false)
...
function 遊戲 () {
    ...
    game.splash("確定之後按住A鍵家往上")
    游標 = sprites.create(assets.image`myImage5`, SpriteKind.Player)
    controller.moveSprite(游標, 100, 100)
    石頭 = sprites.create(assets.image`myImage0`, SpriteKind.stone)
    布 = sprites.create(assets.image`myImage1`, SpriteKind.paper)
    剪刀 = sprites.create(assets.image`myImage7`, SpriteKind.sesire)
    石頭.setPosition(22, 67)
    布.setPosition(55, 67)
    剪刀.setPosition(104, 67)
    if (游標.overlapsWith(石頭)) {
